...
export default function NewRTPWidget({
  color,
  gameObject,
}: {
  color: string;
    ...
    <div className="rounded-xl bg-dark2 p-4 h-full max-sm:rounded-t-[0px] max-sm:p-1">
      {gameObject?.rtp && RTP ? (
        <div className="rounded-xl bg-dark1 h-full flex flex-col items-center pt-4  relative justify-between">
          ...
          <div>
            <div className="relative">
              <svg xmlns="http://www.w3.org/2000/svg" width={210} height={105} fill="none">
                <path
                  fill={`url(#${color})`}
            ...
            <div className="flex justify-center mb-4 mt-8 font-bold ">
              <span className="text-grey1">Slot is in lose </span>
              <span className="text-white">&nbsp;6.97%</span>
              <span className="text-green1">&nbsp;(+2.54%)</span>
            </div>
          </div>
        </div>
      ) : (
        <SkeletonTheme baseColor="#24262C" highlightColor="#444">
          <section>
            <Skeleton count={1} className=" h-[233px] rounded-xl" />
          </section>
        </SkeletonTheme>
      )}
    </div>
