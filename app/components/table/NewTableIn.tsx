...
export default function NewTableIn({
  gamesList,
  onAddToCompare,
  orderBy,
  isFiat,
              ...
              {tableInstance.getRowModel().rows.map((row) => {
                return (
                  <tr
                    ...
                    key={row.id}
                    className={`hover:bg-dark2 cursor-pointer ${row.original.statType === "ApiDriven" && "bg-darkBlue"}`}
                  >
        ...
        <div className=" mt-4 flex items-center flex-col md:flex-row  md:justify-between md:my-8">
          ...
          {!orderBy ? (
            <ReactPaginate
              ...
              previousLabel={<Image src={back} alt="" className="h-4 w-4" height={96} width={96} />}
              nextLabel={
            ...
            <span className=" text-sm text-grey1 font-light max-w-2xl text-center">
              Listing sorted by highest ratings at the moment (5-minute intervals). Maximum 50 games
              shown. Games may vary every 5 minutes as SPS data updates live.
            </span>
          )}
